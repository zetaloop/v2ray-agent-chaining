...
updateV2RayAgent() {
    ...
    if [[ "${release}" == "alpine" ]]; then
        wget -c -q -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/zetaloop/vasma-chaining/master/install.sh"
    else
        wget -c -q "${wgetShowProgressStatus}" -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/zetaloop/vasma-chaining/master/install.sh"
    fi
    ...
    echoContent skyBlue "wget -P /root -N --no-check-certificate https://raw.githubusercontent.com/zetaloop/vasma-chaining/master/install.sh && chmod 700 /root/install.sh && /root/install.sh"
    echo
    exit 0
}
